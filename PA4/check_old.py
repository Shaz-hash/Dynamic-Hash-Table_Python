...
def testJoin(nodes,p):
	...
	if nodes[1].successor == ("localhost",p[0]) and nodes[1].predecessor == ("localhost",p[0]) and nodes[0].successor == ("localhost",p[1]) and nodes[0].predecessor == ("localhost",p[1]):
		...
		print("result are :")
		print(nodes[1].successor)
		print(nodes[0].successor)
...
def testPutandGet(nodes, files):
	...
	for i in range(len(files)):
		...
		nodes[0].put(files[i])
		print("** file name : " , files[i] , " file hash : " , nodes[0].hasher(files[i]) )
	...
	for i in range(len(files)):
		for j in range(len(nodes)):
			if (fileHashes[i] <= nodes[j].key and fileHashes[i] > nodes[j-1].key) or (fileHashes[i] > nodes[-1].key and j == 0):
				...
				print( files[i] , " must be in ",  nodes[j].key )
				...
				if files[i] not in nodes[j].files:
					...
					print(nodes[j].files)
					print("this file : " , files[i] , "is not in port : ", nodes[j].port )
					correct = False
				else :
					print("yes!")
...
def testLeave(nodes, files):
	...
	nodes[ind].leave()
	print("node leaving is : " , nodes[ind].port)
	time.sleep(2)
	del nodes[ind]
	correct = True
	...
	if files[0] in nodes[ind % len(nodes)].files:
		...
		print("checking if this file is in : " , files[0] , nodes[ind % len(nodes)].port )
